...
if(nivel){var nivelParts=nivel.split(" ");if(nivelParts.length>=2){document.querySelector('#sm-home h2 strong').innerHTML=nivelParts[0].replace('º',' ').replace('ª',' ');document.querySelector('#sm-home h2 span').innerHTML=nivel.substr(nivelParts[0].length+1);document.querySelector('#sm-home h2 span').innerHTML=nivel.substr(nivelParts[0].length+1).replace('BACHILLERATO','BTO').replace('bachillerato','BTO').replace('Bachillerato','BTO').replace('º',' ').replace('ª',' ');//Is text is more than 5 chars lengh, we add a class to add padding.
if(document.querySelector('#sm-home h2').innerText.length>5){document.querySelector('#sm-home h2').classList.add('sm-long-text');}}else{document.querySelector('#sm-home h2').style.display='none';}}else{document.querySelector('#sm-home h2').style.display='none';}//Logo
var courseTagAndLogoRelation=[{tag:'Andalucia',logo:'grazalema'},{tag:'Cataluña',logo:'gruppromotor'},{tag:'Galicia',logo:'obradoiro'},{tag:'Valencia',logo:'voramar'},{tag:'Pais Vasco',logo:'zubia'},{tag:'Baleares',logo:'illes_balears'},{tag:'Canarias',logo:'santillana_canarias'}];if(data.courseTags){_.forEach(courseTagAndLogoRelation,function(item){if(data.courseTags.indexOf(item.tag)>-1){var logoElement=document.querySelector('#sm-home .sm-content .sm-content-logosantillana');if(logoElement){logoElement.classList.add(item.logo);}}});}document.querySelector('#sm-home').style.backgroundImage='url('+window.smBookBackgroundImage+')';// document.querySelector('#sm-home .sm-button-next-screen').addEventListener('click', this.onNextButton.bind(this));
