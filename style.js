...
if(typeof this.activitiContentUpdate=='function'){this.activitiContentUpdate();}console.log('activeButtonIndex');console.log(activeButtonIndex);console.log('buttons.length');console.log(buttons.length);//Check arrows state
//Left arrow
if(!window.esPopup&&this.state.isStartSlide||window.esPopup&&activeButtonIndex==0){document.querySelector('.sm-slider-indicator-arrow-left').classList.add('disabled');}else{document.querySelector('.sm-slider-indicator-arrow-left').classList.remove('disabled');}//Right arrow
if(!window.esPopup&&this.state.isEndSlide||window.esPopup&&activeButtonIndex==buttons.length-3){document.querySelector('.sm-slider-indicator-arrow-right').classList.add('disabled');}else{document.querySelector('.sm-slider-indicator-arrow-right').classList.remove('disabled');}}},{key:'onArrowLeftClick',value:function onArrowLeftClick(dontExecuteActionInSlide){var executeActionInSlide=!(dontExecuteActionInSlide===true);// document.querySelectorAll('.slider-indicators li').forEach((element) => {
...
document.querySelector('.navbar.libro').insertAdjacentHTML('afterbegin',replaceStringInTemplates.replace(_subUnitBackButton2.default));document.querySelector('#sm-subunit-backbutton').addEventListener('click',this.onBackButtonClick.bind(this));//If preview
if(blink.activity.slidePreview||window.esPopup){//Si  está en modo preview o popup
//Hide back button and TOC button with a class
document.querySelector('.navbar.libro').classList.add('is-preview');//Hide review buttons
var reviewElements=document.querySelectorAll('.review');_.forEach(reviewElements,function(reviewEl){return reviewEl.style.display='none';});//remove all elements of title less last span.ellipsis
if(document.querySelector('.navbar.libro span.title')){var titleChildren=document.querySelector('.navbar.libro span.title').childNodes;var titleCatched=false;for(var index=titleChildren.length-1;index>=0;index--){var child=titleChildren[index];if(!titleCatched&&child.nodeName=='SPAN'){titleCatched=true;}else{child.remove();}}}}//Activities
