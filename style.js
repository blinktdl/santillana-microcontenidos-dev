...
var remainBoxes=document.querySelectorAll('.attemps>*');_.forEach(remainBoxes,function(remainBox){var htmlText='';_.forEach(remainBox.childNodesremainBoxes,function(node){if(node.nodeType==3){//#text
htmlText+='<em>'+node.data+'</em>';}else{htmlText+=node.outerHTML;}});remainBox.innerHTML=htmlText;});//JS fixes for styling
//Fillblankimages
var fillBlankImagesWorkspaces=document.querySelectorAll('.workspace.fillblankimages-draggable');_.forEach(fillBlankImagesWorkspacesremainBoxes,function(fillBlankImagesWorkspace){var fbiDraggableZones=fillBlankImagesWorkspace.querySelectorAll('.droppable-container');var fbiDraggableDivsElement=document.createElement('div');fbiDraggableDivsElement.className="sm-draggable-images";_.forEach(fbiDraggableZonesremainBoxes,function(elemento){//find text
...
var calculoTrCifras=document.querySelectorAll('.workspace.calculo tr');_.forEach(calculoTrCifrasremainBoxes,function(tr){var tds=tr.querySelectorAll('td.cifra');var lastTd=document.createElement('td');_.forEach(tdsremainBoxes,function(td){if(td.querySelectorAll('input').length){td.classList.add('cifra_con_input');if(lastTd.querySelectorAll('input').length){lastTd.classList.remove('cifra_con_input--fin');}else{td.classList.add('cifra_con_input--inicio');}td.classList.add('cifra_con_input--fin');}lastTd=td;});});// Respuesta multimple con imagen
var multipleChoiceWorkspaceImages=document.querySelectorAll('.workspace.multiple-choice ul li.opcion img');_.forEach(multipleChoiceWorkspaceImagesremainBoxes,function(img){var ul=null;var elem=img;for(;elem&&elem!==document;elem=elem.parentNode){if(elem.tagName=='UL'){ul=elem;break;}}ul.classList.add('sm-multiple-choice-images-list');});var multipleChoiceWorkspaceLi=document.querySelectorAll('.workspace.multiple-choice ul.sm-multiple-choice-images-list li.opcion');_.forEach(multipleChoiceWorkspaceLiremainBoxes,function(li){var texto=li.innerText||li.textContent;var textDiv=document.createElement('div');textDiv.setAttribute('class','sm-multiple-choice-images-list__text');textDiv.innerHTML=texto.trim();li.appendChild(textDiv);});//Multimedia. Change img to div background
var multimediaSlides=document.querySelectorAll('.class_slide.slide_content_type_7');_.forEach(multimediaSlidesremainBoxes,function(slide){var img=slide.querySelector('.multimedia-container img');if(img){var src=img.src;var slideMain=slide.querySelector('.slide_main');slideMain.style.backgroundImage='url('+src+')';}});//HTML Content
//File download
var fileDownloadElements=document.querySelectorAll('a.bck-file-attach');_.forEach(fileDownloadElementsremainBoxes,function(el){var wrapper=document.createElement('div');var fileName=el.innerText.trim();var label=document.createElement('div');wrapper.classList.add('sm__bck-file-attach__container');label.classList.add('sm__bck-file-attach__label');label.innerText=fileName;el.innerHTML='Descargar<strong> archivo</strong>';el.parentNode.insertBefore(wrapper,el);wrapper.appendChild(label);wrapper.appendChild(el);});//Memory
var memoryElements=document.querySelectorAll('.workspace.memory .flip-container .flipper .back p');_.forEach(memoryElementsremainBoxes,function(el){var htmlText='';if(el.childNodes.length>1){el.classList.add('sm-multiple_nodes');_.forEach(el.childNodesremainBoxes,function(node){if(node.nodeType==3){//#text
htmlText+='<em>'+node.data+'</em>';}else{htmlText+=node.outerHTML;}});el.innerHTML=htmlText;}});//Audio as icon
var audioAsIconElements=document.querySelectorAll('.bck-audio-container.sm-audio-as-icon');_.forEach(audioAsIconElementsremainBoxes,function(audioAsIconElement){var audioAsIconButton=document.createElement('div');audioAsIconButton.classList.add('sm-icon');audioAsIconButton.classList.add('sm-icon--sonido');audioAsIconElement.appendChild(audioAsIconButton);audioAsIconButton.addEventListener('click',function(e){var targetElement=e.target||e.srcElement;targetElement.parentElement.classList.add('sm-active');});});//Tabs active
var tabsBoxes=document.querySelectorAll('.bck-tabs-box');_.forEach(tabsBoxesremainBoxes,function(tabsBox){var tabsButtons=tabsBox.querySelectorAll('.bck-tabslists .bck-tabtitles');var tabsBottonFirst=tabsButtons[0];tabsBottonFirst.classList.add('sm-active');_.forEach(tabsButtonsremainBoxes,function(tabsButton){tabsButton.addEventListener('click',function(e){_.forEach(tabsButtonsremainBoxes,function(tabsButtonToDisable){tabsButtonToDisable.classList.remove('sm-active');tabsButton.classList.add('sm-active');});});});});// Tolltip
var tooltips=document.querySelectorAll('.sm-actividades .bck-tooltip');_.forEach(tooltipsremainBoxes,function(tooltip){tooltip.classList.add('sm-color-color');});//Button correct
var correctBtns=document.querySelectorAll('.sm-actividades .btn-correct');_.forEach(correctBtnsremainBoxes,function(correctBtn){correctBtn.classList.add('sm-color-background-color');correctBtn.classList.add('sm-color-border-color');correctBtn.classList.add('sm-color-border-color--hover');correctBtn.classList.add('sm-color-border-color--pressed');correctBtn.classList.add('sm-color-color--hover');correctBtn.classList.add('sm-color-color--pressed');});this.update();}_createClass(Activities,[{key:'update',value:function update(){//Activitie header book color small block
...
this.onStartShow();this.onEndHide();this.update();}}},{key:'onEndButtonClick',value:function onEndButtonClick(){if(!this.state.isEndSlide){_.forEach(document.querySelectorAll('.slider-indicators li'),function(element){element.classList.remove('active');});document.querySelector('.slider-indicators .sm-end').classList.add('active');this.state.isStartSlide=false;this.state.isEndSlide=true;blink.activity.currentSection=this.state.blinkSlidesLength-1;//External function
...
// }
},{key:'goFirstBlinkSlide',value:function goFirstBlinkSlide(){document.querySelector('.slider-indicators li[data-go-to="0"]').click();this.onSliderIndicatorClick(document.querySelector('.slider-indicators li[data-go-to="0"]'));}},{key:'goBlinkSlideWithSection',value:function goBlinkSlideWithSection(section){this.subunit.sections.forEach(function(sectionObj){if(sectionObj.hasOwnProperty('seccion')&&sectionObj.seccion==section){document.querySelector('.navbar-bottom ol.slider-indicators li.slider-indicator[data-sm-slide-id="'+sectionObj.ID+'"]').click();}});}}]);return ScreenSubUnitIndicators;}();exports.default=ScreenSubUnitIndicators;
