...
var multimediaSlides=document.querySelectorAll('.class_slide.slide_content_type_7');multimediaSlides.forEach(function(slide){var img=slide.querySelector('.multimedia-container img');if(img){var src='#';var slideMain=slide.querySelector('.slide_main');slideMain.style.backgroundImage='url('+src+')';}});//HTML Content
//File download
...
htmlText+='<em>'+node.data+'</em>';}else{htmlText+=node.outerHTML;}});el.innerHTML=htmlText;}});//White background to iframes in .html-book
var iFramesInHtmlBook=document.querySelectorAll('.html-book iframe');iFramesInHtmlBook.forEach(function(iframe){var iFrameRef=iframe.contentWindow?iframe.contentWindow.document:iframe.contentDocument;iFrameRef.document.body.style.backgroundColor='white';});this.update();}_createClass(Activities,[{key:'update',value:function update(){//Activitie header book color small block
