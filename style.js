...
// 
var blockElement=blockTemplate.cloneNode(true);blockElement.removeAttribute('id');blockElement.setAttribute('id','block-'+block.id);blockElement.querySelector('.sm-unit__block__title').innerText=block.headerSubUnit.data.title;block.subUnits.forEach(function(subunitData){var subunit=subunitData.data;(0,_debug.cl)("subunit");(0,_debug.cl)(subunit);var activityElement=mainActivityTemplate.cloneNode(true);activityElement.removeAttribute('id');activityElement.setAttribute('id','subunit-'+subunit.id);//Si es alumno y el candado está bloqueado
////l(subunit.lock);
...
// 			cl(`Lanzando función blink.theme.updateVisibleActivity(${subunit.id})`);
...
"use strict";
Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();var _config=__webpack_require__(0);var _config2=_interopRequireDefault(_config);var _responsiveStatus=__webpack_require__(4);var _responsiveStatus2=_interopRequireDefault(_responsiveStatus);var _tinySlider=__webpack_require__(11);__webpack_require__(10);var _replaceStringInTemplates=__webpack_require__(1);var _replaceStringInTemplates2=_interopRequireDefault(_replaceStringInTemplates);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var replaceStringInTemplates=new _replaceStringInTemplates2.default();var ScreenUnitsMenu=function(){function ScreenUnitsMenu(){_classCallCheck(this,ScreenUnitsMenu);this.state={started:false};this.slider=null;this.goto=null;this.units=[];this.responsiveStatus=null;}_createClass(ScreenUnitsMenu,[{key:'init',value:function init(_ref){var data=_ref.data,goTo=_ref.goTo;if(!this.started){this.goTo=goTo;this.units=data.units;document.querySelector('#sm-units-menu').classList.add('active');// document.querySelector('#sm-home h1').innerHTML = title;
...
(0,_debug.cl)(this.data.units[6].subunitsmain);//First, we search blocks
_.forEach(this.data.units,function(unit){unit.blocks=[];_.forEach(unit.subunits,function(subunit){var tags=subunit.tags.split(' ');var isBlock=_.includes(tags,'sm_metodologia_titulo_bloque');var metodologiaBlockTag=_.find(tags,function(tag){// :D sorry...
...
_.forEach(this.data.units,function(unit){_.forEach(unit.subunits,function(subunit){var tags=subunit.tags.split(' ');var isBlock=_.includes(tags,'sm_metodologia_titulo_bloque');var metodologiaBlockTag=_.find(tags,function(tag){// :D sorry...
return tag.substr(0,15)==='sm_metodologia_'&&tag!=='sm_metodologia_titulo_bloque';});var metodologiaBlockId=metodologiaBlockTag?metodologiaBlockTag.replace('sm_metodologia_',''):null;var isNewKidOnTheBlock=!isBlock&&metodologiaBlockTag?true:false;if(isNewKidOnTheBlock){var currentBlock=_.find(unit.blocks,{id:metodologiaBlockId});if(currentBlock){var newKidOnTheBlock={data:subunit};currentBlock.subUnits.push(newKidOnTheBlock);}}});});(0,_debug.cl)(this.data.units[6]);//Regenerate subunints numbers depending of ocultar
