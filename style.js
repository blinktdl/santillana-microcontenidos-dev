...
var notConcatenatedCounter=0;_lodash2.default.forEach(this.subunit.sections,function(section){if(section.concatenate!="1"){var allButtons=document.querySelectorAll('.navbar-bottom ol.slider-indicators li.slider-indicator');var currentButton=_lodash2.default.find(allButtons,function(button){return button.dataset.goTo==notConcatenatedCounter;});if(currentButton){currentButton.setAttribute('data-sm-slide-id',section.ID);}notConcatenatedCounter++;}});// Add start and end buttons
...
$('.slider-indicators li[data-go-to="0"]').trigger('click');this.onSliderIndicatorClick(document.querySelector('.slider-indicators li[data-go-to="0"]'));}},{key:'goBlinkSlideWithSection',value:function goBlinkSlideWithSection(section){_lodash2.default.forEach(this.subunit.sections,function(sectionObj){if(sectionObj.hasOwnProperty('seccion')&&sectionObj.seccion==section){// document.querySelector(`.navbar-bottom ol.slider-indicators li.slider-indicator[data-sm-slide-id="${sectionObj.ID}"]`).click();
