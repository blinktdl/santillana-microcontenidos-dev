...
_.forEach(document.querySelectorAll('#slider .item-container .header'),function(headerDomElement){var titleString=headerDomElement.querySelector('.title h3').innerText;if(titleString.substr(0,2)==='##'){headerDomElement.classList.add('header--no-title');}});//Next button when has more than one line
var nextButtonsUpdate=function nextButtonsUpdate(){_.forEach(document.querySelectorAll('#slider .nextbutton-warp .nextbutton-content'),function(nextButton){nextButton.classList.remove('nextbutton-content--two-lines');var pHeight=nextButton.clientHeight;if(pHeight>81){nextButton.classList.add('nextbutton-content--two-lines');}});};window.addEventListener('resize',function(){nextButtonsUpdate();},false);nextButtonsUpdate();this.update();}_createClass(Activities,[{key:'update',value:function update(){////l('Activities.update()');
...
// Elminidado según https://trello.com/c/WNj3nsCm
// if(nivel){
// 	let nivelParts = nivel.split(" ");
// 	if(nivelParts.length >= 2){
// 		document.querySelector('#sm-home h2 strong').innerHTML = nivelParts[0];
// 		document.querySelector('#sm-home h2 span').innerHTML = nivel.substr(nivelParts[0].length + 1);
// 		document.querySelector('#sm-home h2 span').innerHTML = nivel.substr(nivelParts[0].length + 1).replace('BACHILLERATO', 'BTO').replace('bachillerato', 'BTO').replace('Bachillerato', 'BTO'); //.replace('SECONDARY', 'BTO').replace('secondary', 'BTO').replace('Secondary', 'BTO').replace('Baccalaureate', 'BTO');
// 	} else {
// 		document.querySelector('#sm-home h2').style.display = 'none';
// 	}
// } else {
// 	document.querySelector('#sm-home h2').style.display = 'none';
// }
document.querySelector('#sm-home').style.backgroundImage='url('+window.smBookBackgroundImage+')';// document.querySelector('#sm-home .sm-button-next-screen').addEventListener('click', this.onNextButton.bind(this));
