...
this.screenSubUnitStart.init(this.subUnitObject,this.number,this.isEvaluacion);this.screenSubUnitStart.goFirstBlinkSlide=function(){return _this.screenSubUnitIndicators.goFirstBlinkSlide();};this.screenSubUnitStart.onGoToActivity=this.onGoToActivity.bind(this);this.screenSubUnitStart.onGoToActivityById=this.onGoToActivityById.bind(this);//EndPage
...
this.activities=new _Activities2.default();this.screenSubUnitIndicators.activitiContentUpdate=this.activities.update;}this.started=true;}},{key:'onGoToActivity',value:function onGoToActivity(page){this.state.inActivity=true;document.querySelector('.slider-indicators li[data-go-to="'+(parseInt(page)-1)+'"]').click();}},{key:'onGoToActivityById',value:function onGoToActivityById(ID){this.state.inActivity=true;document.querySelector('.slider-indicators li[data-sm-slide-id="'+ID+'"]').click();}},{key:'onBackButtonClick',value:function onBackButtonClick(){if(!this.isEvaluacion){// Chec if we are in secuencia o actividades
