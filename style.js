...
// 	}
// });
if((typeof blink==='undefined'?'undefined':_typeof(blink))=='object'&&blink.isApp){activityElement.getElementsByTagName('a')[0].addEventListener('click',function(){eval(subunit.onclickTitle);});activityElement.getElementsByTagName('a')[0].setAttribute('href','javascript:void()');}else{activityElement.getElementsByTagName('a')[0].setAttribute('href',''+subunit.url);}activityElement.querySelector('.sm-unit__activity_aside__image').style.backgroundImage='url('+subunit.image+')';if(subunit.smtype){activityElement.querySelector('.sm-unit__activity_aside__title--con-tag').innerHTML=subunit.title;var tipoTexto='';switch(subunit.smtype){case'flipped':tipoTexto='Flipped Classroom';break;case'microproyecto':tipoTexto='Microproyecto';break;case'basica':tipoTexto='Básica';break;default:break;}activityElement.querySelector('.sm-unit__activity_aside__type--con-tag').innerHTML=replaceStringInTemplates.replace('--Metodología--')+' <strong>'+tipoTexto+'</strong>';// activityElement.querySelector('.sm-unit__activity_aside__description--con-tag').innerHTML = subunit.description;
