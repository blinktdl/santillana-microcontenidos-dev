...
if(nivel){var nivelParts=nivel.split(" ");if(nivelParts.length>=2){document.querySelector('#sm-home h2 strong').innerHTML=nivelParts[0];document.querySelector('#sm-home h2 span').innerHTML=nivel.substr(nivelParts[0].length+1);//document.querySelector('#sm-home h2 span').innerHTML = nivelParts[1].replace('BACHILLERATO', 'BTO').replace('bachillerato', 'BTO').replace('Bachillerato', 'BTO').replace('SECONDARY', 'BTO').replace('secondary', 'BTO').replace('Secondary', 'BTO').replace('Baccalaureate', 'BTO');
