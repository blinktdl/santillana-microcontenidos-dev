...
})(window, function(__WEBPACK_EXTERNAL_MODULE__53__) {
...
// 	break;
case'basica':tipoTexto='Básica';break;default:break;}tipoTexto=replaceStringInTemplates.replace(tipoTexto);document.querySelector('.sm-subunit-start__secuencia__content span').innerHTML=replaceStringInTemplates.replace('--Metodología--')+' <strong>'+tipoTexto+'</strong>';}else{document.querySelector('.sm-subunit-start__secuencia__content span').innerHTML=subUnitObject.description;}// if(document.querySelector(`.sm-subunit-start__secuencia__content svg.${subUnitObject.smtype}-icon`)){
...
var Dropdown=function(){function Dropdown(){_classCallCheck(this,Dropdown);this.state={lodaded:false,active:false};this.domElement;}_createClass(Dropdown,[{key:'init',value:function init(domElement){if(!this.state.loaded){this.state.loaded=true;this.domElement=domElement;this.domElement.firstElementChild.addEventListener('click',this.onButtonClick.bind(this));//If click outside
var _this=this;document.addEventListener('click',function(e){var isClickInside=_this.domElement.contains(e.target);if(!isClickInside){_this.onClose();}});}}},{key:'onButtonClick',value:function onButtonClick(){if(this.state.active){this.onClose();}else{this.onOpen();}}},{key:'onClose',value:function onClose(){this.domElement.classList.remove('active');this.state.active=false;}},{key:'onOpen',value:function onOpen(){this.domElement.classList.add('active');this.state.active=true;}}]);return Dropdown;}();exports.default=Dropdown;
...
if(this.unit.resourcesmain.length>0){headerElement.querySelector('.sm-unit__header__resources_button').style.display='block';headerElement.querySelector('.sm-unit__header__resources_button').addEventListener('click',function(){var scrollElement=document.querySelector('.sm-content--scrolled');var resourcesContent=scrollElement.querySelector('.sm-unit__activities__aside_container');var resourcesContentRect=resourcesContent.getBoundingClientRect();var scrollTop=window.pageYOffset||document.documentElement.scrollTop;var resourcesContentTop=resourcesContentRect.top+scrollTop-64;(0,_animejs2.default)({targets:scrollElement,scrollTop:resourcesContentTop,duration:500,easing:'easeInOutQuad'});});}else{headerElement.querySelector('.sm-unit__header__resources_button').style.display='none';}// Main activities
...
var _config=__webpack_require__(0);var _config2=_interopRequireDefault(_config);var _urlHelper=__webpack_require__(47);var _urlHelper2=_interopRequireDefault(_urlHelper);var _Transitions=__webpack_require__(46);var _Transitions2=_interopRequireDefault(_Transitions);var _debug=__webpack_require__(1);var _1ScreenHome=__webpack_require__(44);var _1ScreenHome2=_interopRequireDefault(_1ScreenHome);var _2ScreenUnitsMenu=__webpack_require__(41);var _2ScreenUnitsMenu2=_interopRequireDefault(_2ScreenUnitsMenu);var _3ScreenUnit=__webpack_require__(37);var _3ScreenUnit2=_interopRequireDefault(_3ScreenUnit);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}//El hash puede ser
// "#" o "#" => Home
