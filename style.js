...
this.unit.blocks.forEach(function(block){(0,_debug.cl)('block');(0,_debug.cl)(block);// 
var blockElement=blockTemplate.cloneNode(true);blockElement.removeAttribute('id');blockElement.setAttribute('id','block-'+block.id);blockElement.querySelector('.sm-unit__block__title').innerText=block.headerSubUnit.data.title;if(block.headerSubUnit.data.title.trim().length===0){blockElement.classList.add('sm-unit__block--no-title');}block.subUnits.forEach(function(subunitData){var subunit=subunitData.data;//l("subunit")
//l(subunit)
