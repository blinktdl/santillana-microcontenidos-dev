...
var asideActivitiesContainer=unitElement.querySelector('.sm-unit__activities__aside');while(asideActivitiesContainer.firstChild){asideActivitiesContainer.removeChild(asideActivitiesContainer.firstChild);}var asideActivityTemplate=document.querySelector('#sm-unit__activity_aside--template');this.unit.resourcesmain.forEach(function(subunit,index){var activityElement=asideActivityTemplate.cloneNode(true);activityElement.removeAttribute('id');var svgElements=activityElement.querySelectorAll('.sm-unit__activity_aside__icon svg');_.forEach(svgElements,function(svgElement){if(!svgElement.classList.contains(subunit.smtype+'-icon')){svgElement.remove();}});// activityElement.getElementsByTagName('a')[0].setAttribute('href',`#activity_${subunit.id}`);
...
var asideClassesName=['flipped','refuerzo','microproyecto','ampliacion','teacher','discover'];var evaluationClassesName=['evaluacion'];data.units.forEach(function(unit){unit.subunitsmain=[];unit.subunitsevaluation=[];unit.resourcesmain=[];unit.resourcesevaluation=[];unit.subunits.forEach(function(subunit){var added=false;if(subunit.tag){evaluationClassesName.forEach(function(tagName){if(subunit.tag.indexOf(tagName)>=0){if(!added){//Search if parent SubUnit exists
