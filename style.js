...
if(activities[subunit.id]){subunit.completado=parseInt(activities[subunit.id].avance);//Borja me dijo completado, pero no encuentro esa propiedad
}else{subunit.completado=0;//No sé si es necesario
}unitCompletedTotal+=subunit.completado;});unit.resources.forEach(function(resource){if(activities[resource.id]){resource.completado=parseInt(activities[resource.id].avance);//Borja me dijo completado, pero no encuentro esa propiedad
