...
});var smStylesEditInit=function smStylesEditInit(data){console.log('%c Santillana Microcontenidos ','font-size: 40px; font-weight: 700; color: white; background-color: #2a495d;');console.log('Fecha de compilación: 2020-2-4 4:40:37 PM.');console.log("Iniciando...");console.log("Datos cargados.");//indow.losdatos = data;
...
var menuLeftTitlesForEyes=document.querySelectorAll('#indice .units ul#list-units>li');menuLeftTitlesForEyes.forEach(function(titleItem){var titleString=titleItem.querySelector('.title').innerHTML.trim();if(titleString.slice(-3)==='(P)'){titleItem.classList.add('sm-unit--only-teacher');}});}else{var editarActivitiesNavbar=new _editar_activities_navbar2.default();editarActivitiesNavbar.init(data);}//Activities
