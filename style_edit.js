...
"use strict";
Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();var _lodash=__webpack_require__(3);var _lodash2=_interopRequireDefault(_lodash);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var EditarActivitiesNavbar=function(){function EditarActivitiesNavbar(){_classCallCheck(this,EditarActivitiesNavbar);}_createClass(EditarActivitiesNavbar,[{key:'init',value:function init(data){this.sections=this.findActivitySectionsFromData(data);this.updateNavBar();this.navbarObserver=new MutationObserver(this.checkIfHasChanged.bind(this));this.navbarObserver.observe(document.querySelector('.edit .navbar-bottom .slider-indicators'),{childList:true});}//Selecciona las secciones (slides) de la actividad actual
},{key:'findActivitySectionsFromData',value:function findActivitySectionsFromData(data){var unit=_lodash2.default.find(data.units,{id:idtema.toString()});var subunit=_lodash2.default.find(unit.subunits,{id:idclase.toString()});return subunit.sections;}//Chequea si el contenido ha cambiado de verdad, y no es un tooltip
},{key:'checkIfHasChanged',value:function checkIfHasChanged(mutationsList,observer){var needsUpdate=false;var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=mutationsList[Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var mutation=_step.value;if(mutation.type==='childList'){mutation.addedNodes.forEach(function(node){if(node.classList.contains('slider-indicator')){needsUpdate=true;}});mutation.removedNodes.forEach(function(node){if(node.classList.contains('slider-indicator')){needsUpdate=true;}});}}}catch(err){_didIteratorError=true;_iteratorError=err;}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return();}}finally{if(_didIteratorError){throw _iteratorError;}}}if(needsUpdate){console.log("SE ACTUALIZA");this.loadActivityData();}}//Actualiza los datos (se ejecuta cuando se cambia .slide-indicators)
},{key:'loadActivityData',value:function loadActivityData(){console.log("LOAD");var _this=this;blink.getCourse(idcurso).done(function(data){console.log("LOADED");_this.sections=_this.findActivitySectionsFromData(data);console.log(_this);console.log(_this.sections);_this.updateNavBar();});}},{key:'updateNavBar',value:function updateNavBar(){var _this2=this;var slideIndicatorsElements=document.querySelectorAll('.edit .navbar-bottom .slider-indicators .slider-indicator');console.log(this.sections);_lodash2.default.forEach(slideIndicatorsElements,function(slideElement,i){if(_this2.sections[i]&&_this2.sections[i].tipo&&_this2.sections[i].tipo===7){slideElement.classList.add('sm-multimedia');}else{slideElement.classList.remove('sm-multimedia');}});}}]);return EditarActivitiesNavbar;}();exports.default=EditarActivitiesNavbar;
...
});var smStylesEditInit=function smStylesEditInit(data){console.log('%c Santillana Microcontenidos ','font-size: 40px; font-weight: 700; color: white; background-color: #2a495d;');console.log('Fecha de compilación: 2020-1-17 12:12:36 PM.');console.log("Iniciando...");console.log("Datos cargados.");//indow.losdatos = data;
