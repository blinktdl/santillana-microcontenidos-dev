...
document.querySelector('.navbar.libro .libro-left>ul').insertAdjacentHTML('afterbegin',replaceStringInTemplates.replace(_subUnitBackButton2.default));if(blink.isIosApp||blink.isAndroidApp){document.querySelector('#sm-subunit-backbutton').innerHTML='<a href="javascript:blink.rest.closeWindow()"></a>';}else{document.querySelector('#sm-subunit-backbutton').innerHTML='<a href="javascript:closeActivity()"></a>';}}else{var editarActivitiesNavbar=new _editar_activities_navbar2.default();editarActivitiesNavbar.init(data);//BackButton
document.querySelector('.navbar.libro .libro-left>ul').insertAdjacentHTML('afterbegin',replaceStringInTemplates.replace(_subUnitBackButton2.default));var bookUrlEditar=data.url.replace('editar=0','editar=1');document.querySelector('#sm-subunit-backbutton').innerHTML='<a href="'+bookUrlEditar+'"></a>';}//Activities
