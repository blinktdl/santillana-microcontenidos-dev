...
block.containerDomElement.querySelector('.sm-editar-block__header').dataset['id']=block.headerSubUnit.data.id;//remove headerSubunit. We don't need it
...
var containerActions=document.createElement('div');containerActions.classList.add('sm-editar-block__actions');containerActions.innerHTML='<a><span>'+replaceStringInTemplates.replace('--santillana_microcontenidos_add_activity--')+'</span></a>';containerActions.querySelector('a').addEventListener('click',function(){var url=new URL(window.location.href);var idgrupo=url.searchParams.get('idgrupo')||'';var idtema=url.searchParams.get('idtema')||'';var isResource='false';var type='';var blocktag='sm_metodologia_'+block.id;dlgCreateNewEditClass(idcurso,idtema,null,null,null,isResource,type,idgrupo,blocktag);});containerContent.appendChild(containerActions);//Insert content element in block
